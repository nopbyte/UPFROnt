...
var Policy = (function() {
    ...
    cls.prototype = {
        ...
        checkIncoming : function(trgPolicy, context) {
            ...
            for(var f in dataPolicy.flows) {
                ...
                // ignore all policies which may prevent writing to the item itself
            ...
            for(var f in trgPolicy.flows) {
                ...
                var conflicts = flow.getClosedLocks(tmpContext, flow.source.type);
        ...
        checkFlow : function(policy, direction, context) {
            ...
            case Policy.Direction.INCOMING:
                {
                    result = this.checkIncoming(policy, context);
                    break;
                }
            case Policy.Direction.OUTGOING:
                {
                    result = this.checkOutgoing(policy, context);
                    break;
                }
            default:
                {
        ...
        checkAccess : function(subject, operation, context) {
            ...
            case Policy.Operation.DEL:
                throw new Error("Operation in Policy::checkAccess not implemented yet!");
                break;
            default:
                throw new Error("Unknown operation in Policy::checkAccess!");
            }
        ...
        checkWrite : function(writer, context) {
            ...
            var source = new Entity({ type : context.subject.type });
        ...
        checkRead : function(reader, context) {
            ...
            var target = new Entity( { type : context.subject.type } );
            ...
            for(var f in this.flows) {
                ...
                // flowSrc is less specific than the writer
                if(flowTrg.dominates(target)) {

                    var conflicts = flow.getClosedLocks(context, flow.target.type);
